...
func (nc *netConn) Write(p []byte) (int, error) {
	...
	if nc.writeExpired.Load() == 1 {
		return 0, WrapConnectionError(context.DeadlineExceeded, "write")
	}
...
func (nc *netConn) read(p []byte) (int, error) {
	if nc.readExpired.Load() == 1 {
		return 0, WrapConnectionError(context.DeadlineExceeded, "read")
	}
	...
	if nc.reader == nil {
		...
		if typ != nc.msgType {
			err := WrapProtocolViolation("unexpected frame type read (expected %v): %v", nc.msgType, typ)
