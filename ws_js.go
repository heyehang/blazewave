...
import (
	...
	"github.com/heyehang/blazewave/internal/wsjs"
)
...
func (c *Conn) close(err error, wasClean bool) {
	c.closeOnce.Do(func() {
		...
		if !wasClean {
			err = WrapConnectionError(err, "unclean connection close")
		}
		c.setCloseErr(err)
...
func (c *Conn) init() {
	...
	c.releaseOnClose = c.ws.OnClose(func(e wsjs.CloseEvent) {
		...
		// it's possible the browser triggered it without us
		// explicitly sending it.
		c.close(err, e.WasClean)

		// Event: disconnect
		if c.event != nil {
			_ = c.event.handleDisconnect(context.Background(), c)
		}
	...
	c.releaseOnError = c.ws.OnError(func(v js.Value) {
		...
		// Event: error
		if c.event != nil {
			_ = c.event.handleError(context.Background(), c, errors.New(v.Get("message").String()))
		}
	})
	...
	c.releaseOnMessage = c.ws.OnMessage(func(e wsjs.MessageEvent) {
		...
		// Event: message (text/binary)
		if c.event != nil {
			ctx := context.Background()
			switch p := e.Data.(type) {
			case string:
				_ = c.event.handleMessage(ctx, c, MessageText, []byte(p))
			case []byte:
				_ = c.event.handleMessage(ctx, c, MessageBinary, p)
			}
		}

		// Notify the read goroutine there is something in readBuf.
		select {
	...
	// Event: connect
	if c.event != nil {
		_ = c.event.handleConnect(context.Background(), c)
	}
}
...
func (c *Conn) Read(ctx context.Context) (MessageType, []byte, error) {
	...
	if err != nil {
		return 0, nil, WrapConnectionError(err, "read")
	}
	readLimit := c.msgReadLimit.Load()
	if readLimit >= 0 && int64(len(p)) > readLimit {
		err := WrapProtocolViolation("read limited at %v bytes", c.msgReadLimit.Load())
...
// WriteControl is mocked out for Wasm.
...
func (c *Conn) Write(ctx context.Context, typ MessageType, p []byte) error {
	...
	if err != nil {
		...
		// MessageText.
		err := WrapConnectionError(err, "write")
		c.setCloseErr(err)
		c.closeWithInternal()
		return err
	}
...
func (c *Conn) write(typ MessageType, p []byte) error {
	...
	default:
		return WrapProtocolViolation("unexpected message type: %v", typ)
	}
}
...
func (c *Conn) Close(code StatusCode, reason string) error {
	...
	if err != nil {
		return WrapConnectionError(err, "close WebSocket")
	}
	return nil
}
...
func Dial(ctx context.Context, url string, opts *DialOptions) (*Conn, *http.Response, error) {
	...
	if err != nil {
		return nil, nil, WrapConnectionError(err, fmt.Sprintf("WebSocket dial %q", url))
	}
	return c, resp, nil
}
...
func (w *writer) Write(p []byte) (int, error) {
	...
	n, err := w.b.Write(p)
	if err != nil {
		return n, WrapConnectionError(err, "write message")
	}
	return n, nil
}
...
func (w *writer) Close() error {
	...
	if err != nil {
		return WrapConnectionError(err, "close writer")
	}
	return nil
}
...
func (c *Conn) setCloseErr(err error) {
	c.closeErrOnce.Do(func() {
		c.closeErr = WrapConnectionError(err, "WebSocket closed")
	})
}
