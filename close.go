...
func (c *Conn) writeClose(code StatusCode, reason string) error {
	...
	if ce.Code != StatusNoStatusRcvd {
		p, err = ce.bytes()
		if err != nil {
			return WrapConnectionError(err, "marshal close frame")
		}
	}
	...
	if err != nil && !errors.Is(err, ErrNetworkClosed) {
		return err
	}
	return nil
}
...
func parseClosePayload(p []byte) (CloseError, error) {
	...
	if len(p) < 2 {
		return CloseError{}, WrapProtocolViolation("close payload %q too small, cannot even contain the 2 byte status code", p)
	}
	...
	if !validWireCloseCode(ce.Code) {
		return CloseError{}, WrapProtocolViolation("invalid status code %v", ce.Code)
	}
...
func (ce CloseError) bytes() ([]byte, error) {
	p, err := ce.bytesErr()
	if err != nil {
		err = WrapConnectionError(err, "marshal close frame")
		ce = CloseError{
...
func (ce CloseError) bytesErr() ([]byte, error) {
	if len(ce.Reason) > maxCloseReason {
		return nil, WrapProtocolViolation("reason string max is %v but got %q with length %v", maxCloseReason, ce.Reason, len(ce.Reason))
	}

	if !validWireCloseCode(ce.Code) {
		return nil, WrapProtocolViolation("status code %v cannot be set", ce.Code)
	}
