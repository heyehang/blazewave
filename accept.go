...
func accept(w http.ResponseWriter, r *http.Request, opts *AcceptOptions) (_ *Conn, err error) {
	...
	if err != nil {
		err = WrapConnectionError(err, "hijack connection")
	...
	var (
		br, bw, brBuf, bwBuf = initAcceptReaderWriter(netConn, opts)
	)
...
func verifyClientRequest(w http.ResponseWriter, r *http.Request) (errCode int, _ error) {
	if !r.ProtoAtLeast(1, 1) {
		return http.StatusUpgradeRequired, WrapProtocolViolation("handshake request must be at least HTTP/1.1: %q", r.Proto)
	}

	if !headerContainsTokenIgnoreCase(r.Header, "Connection", "Upgrade") {
		w.Header().Set("Connection", "Upgrade")
		w.Header().Set("Upgrade", "websocket")
		return http.StatusUpgradeRequired, WrapProtocolViolation("Connection header %q does not contain Upgrade", r.Header.Get("Connection"))
	}

	if !headerContainsTokenIgnoreCase(r.Header, "Upgrade", "websocket") {
		w.Header().Set("Connection", "Upgrade")
		w.Header().Set("Upgrade", "websocket")
		return http.StatusUpgradeRequired, WrapProtocolViolation("Upgrade header %q does not contain websocket", r.Header.Get("Upgrade"))
	}

	if r.Method != "GET" {
		return http.StatusMethodNotAllowed, WrapProtocolViolation("handshake request method is not GET but %q", r.Method)
	}

	if r.Header.Get("Sec-WebSocket-Version") != "13" {
		w.Header().Set("Sec-WebSocket-Version", "13")
		return http.StatusBadRequest, WrapProtocolViolation("unsupported WebSocket protocol version (only 13 is supported): %q", r.Header.Get("Sec-WebSocket-Version"))
	}
	...
	if v, err := base64.StdEncoding.DecodeString(websocketSecKey); err != nil || len(v) != 16 {
		return http.StatusBadRequest, WrapProtocolViolation("invalid Sec-WebSocket-Key %q, must be a 16 byte base64 encoded string", websocketSecKey)
	}
...
func authenticateOrigin(r *http.Request, originHosts []string) error {
	...
	if err != nil {
		return WrapConnectionError(err, fmt.Sprintf("parse Origin header %q", origin))
	}
	...
	for _, hostPattern := range originHosts {
		...
		if err != nil {
			return WrapConnectionError(err, fmt.Sprintf("parse path pattern %q", hostPattern))
		}
		if matched {
			return nil
		}
	...
	if u.Host == "" {
		return WrapProtocolViolation("request Origin %q is not a valid URL with a host", origin)
	}
	return WrapProtocolViolation("request Origin %q is not authorized for Host %q", u.Host, r.Host)
}
...
func initAcceptReaderWriter(netConn io.ReadWriter, opts *AcceptOptions) (*bufio.Reader, *bufio.Writer, *pool.Buffer, *pool.Buffer) {
	var (
		br           *bufio.Reader
		bw           *bufio.Writer
