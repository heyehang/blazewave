...
// It returns the type of the message and the payload.
//
// The maximum message size is 32768 bytes.
// See SetReadLimit to change this.
...
func (mr *msgReader) resetFlate() {
	...
	if mr.flateBufio == nil {
		if mr.c.brPool != nil {
			mr.flateBuf = mr.c.brPool.Get()
			mr.flateBufio = bufio.NewReaderSize(mr.readFunc, len(mr.flateBuf.Bytes()))
			mr.flateBufio.ResetBuffer(mr.readFunc, mr.flateBuf.Bytes())
		} else {
			mr.flateBufio = bufio.NewReader(mr.readFunc)
		}
	}
...
func (c *Conn) readLoop(ctx context.Context) (header, error) {
	for {
		...
		if h.rsv1 && c.readRSV1Illegal(h) || h.rsv2 || h.rsv3 {
			err := WrapProtocolViolation("received header with unexpected rsv bits set: %v:%v:%v", h.rsv1, h.rsv2, h.rsv3)
		...
		case opClose, opPing, opPong:
			...
			if err != nil {
				...
				return header{}, WrapConnectionError(err, fmt.Sprintf("handle control frame %v", h.opcode))
			}
		case opText, opBinary:
			c.updateLastPongTime()

			if c.event != nil {
				b, err := c.readMessagePayload(ctx, h)
				if err != nil {
					return header{}, err
				}

				if err := c.event.handleMessage(ctx, c, MessageType(h.opcode), b); err != nil {
					return header{}, WrapConnectionError(err, fmt.Sprintf("handle message frame %v", h.opcode))
				}
				continue
			}
		...
		case opContinuation:
			return h, nil
		default:
			err := WrapProtocolViolation("received unknown opcode %v", h.opcode)
			c.writeError(StatusProtocolError, err)
			return header{}, err
		}
	}
...
func (c *Conn) readFramePayload(ctx context.Context, p []byte) (_ int, err error) {
	...
	if err == nil && c.closedFlag.Load() {
		return n, ErrNetworkClosed
	}
	if err != nil {
		return n, WrapConnectionError(err, "read frame payload")
	}

	return n, err
}

// readMessagePayload reads the message payload from the connection.
func (c *Conn) readMessagePayload(ctx context.Context, h header) ([]byte, error) {
	if h.payloadLength < 0 || h.payloadLength > maxControlPayload {
		err := WrapProtocolViolation("received message frame payload with invalid length: %d", h.payloadLength)
		c.writeError(StatusProtocolError, err)
		return nil, err
	}

	if !h.fin {
		err := errors.New("received fragmented message frame")
		c.writeError(StatusProtocolError, err)
		return nil, err
	}
...
func (c *Conn) handleControl(ctx context.Context, h header) (err error) {
	...
	case opPing:
		if c.event != nil {
			if err := c.event.handlePing(ctx, c, b); err != nil {
				return err
			}
			return nil
		}
	...
	case opPong:
		...
		if c.event != nil {
			if err := c.event.handlePong(ctx, c, b); err != nil {
				return err
			}
		}
	...
	case opClose:
		...
		if err != nil {
			err = WrapConnectionError(err, "received invalid close payload")
			c.writeError(StatusProtocolError, err)
			return err
		}

		if c.event != nil {
			_ = c.event.handleClose(ctx, c, ce.Code, ce.Reason)
		}

		err = WrapConnectionError(ce, "received close frame")
		c.closeStateMu.Lock()
	...
	default:
		// should not reach here, because handleControl only handles control frames
		return WrapProtocolViolation("unexpected opcode in handleControl: %v", h.opcode)
	}
}
...
func (mr *msgReader) Read(p []byte) (n int, err error) {
	...
	if err != nil {
		return 0, WrapConnectionError(err, "read message")
	}
	...
	if err != nil {
		return n, WrapConnectionError(err, "read message")
	}
	return n, nil
}
...
func (lr *limitReader) Read(p []byte) (int, error) {
	...
	if lr.n == 0 {
		err := WrapProtocolViolation("read limited at %v bytes", lr.limit.Load())
