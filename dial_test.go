...
func TestBadDials(t *testing.T) {
	...
	t.Run("badReq", func(t *testing.T) {
		...
		for _, tc := range testCases {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				_, _, err := blazewave.ExportedDial(ctx, tc.url, tc.opts, tc.rand)
				assert.Error(t, err)
			})
		}
	})
	...
	t.Run("badResponse", func(t *testing.T) {
		...
		_, _, err := blazewave.Dial(ctx, "ws://example.com", &blazewave.DialOptions{
		...
		assert.Contains(t, err.Error(), "expected handshake response status code 101 but got 0")
	})
	...
	t.Run("badBody", func(t *testing.T) {
		...
		rt := func(r *http.Request) (*http.Response, error) {
			...
			h.Set("Sec-WebSocket-Accept", blazewave.SecWebSocketAccept(r.Header.Get("Sec-WebSocket-Key")))
...
func Test_verifyHostOverride(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			c, _, err := blazewave.Dial(ctx, "ws://example.com", &blazewave.DialOptions{
				HTTPClient: mockHTTPClient(rt),
				Host:       tc.host,
			})
...
func Test_verifyServerHandshake(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			key, err := blazewave.SecWebSocketKey(rand.Reader)
			assert.Success(t, err)
			r.Header.Set("Sec-WebSocket-Key", key)

			if resp.Header.Get("Sec-WebSocket-Accept") == "" {
				resp.Header.Set("Sec-WebSocket-Accept", blazewave.SecWebSocketAccept(key))
			}
			...
			_, err = blazewave.VerifyServerResponse(opts, blazewave.CompressionModeOpts(opts.CompressionMode), key, resp)
			if tc.success {
				assert.Success(t, err)
			} else {
				assert.Error(t, err)
...
func TestDialRedirect(t *testing.T) {
	...
	assert.Equal(t, "protocol error", true, blazewave.IsProtocolError(err))
	assert.Contains(t, err.Error(), "Upgrade header \"meow\" does not contain websocket")
}
...
func TestDialViaProxy(t *testing.T) {
	...
	defer cancel()
	c, resp, err := blazewave.Dial(ctx, s.URL, &blazewave.DialOptions{
...
func TestDial_BufferPoolReuse(t *testing.T) {
	...
	opts := &blazewave.DialOptions{
		...
		HTTPClient: mockHTTPClient(func(r *http.Request) (*http.Response, error) {
			...
			return &http.Response{
				StatusCode: http.StatusSwitchingProtocols,
				Header:     h,
				Body:       mockBody{bytes.NewBufferString("")},
			}, nil
		}),
	}
	...
	c, _, err := blazewave.Dial(ctx, "ws://example.com", opts)
	if err != nil {
...
func TestDial_BufferPoolStress(t *testing.T) {
...
func TestDial_BufferPoolStress_Extended(t *testing.T) {
	cases := []struct {
		poolSize int
		bufSize  int
		conns    int
