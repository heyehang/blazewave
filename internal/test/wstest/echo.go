...
func EchoLoop(ctx context.Context, c *blazewave.Conn) error {
...
func Echo(ctx context.Context, c *blazewave.Conn, max int) error {
	expType := blazewave.MessageBinary
	if xrand.Bool() {
		expType = blazewave.MessageText
	}
...
func randMessage(typ blazewave.MessageType, n int) []byte {
	if typ == blazewave.MessageBinary {
